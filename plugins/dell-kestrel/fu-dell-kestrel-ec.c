...
typedef struct __attribute__((packed)) { /* nocheck:blocked */
	struct FuDellKestrelV2DockInfoHeader {
		guint8 total_devices;
		guint8 first_index;
		guint8 last_index;
	} header;
	struct FuDellKestrelEcQueryEntry {
		struct FuDellKestrelV2EcAddrMap {
			guint8 location;
			guint8 device_type;
			guint8 sub_type;
			guint8 arg;
...
fu_dell_kestrel_ec_is_dev_present(FuDevice *device,
				  ...
				  guint8 instance)
{
	return fu_dell_kestrel_ec_dev_entry(device, dev_type, sub_type, instance) != NULL;
}
				     ...
				     guint8 instance)
{
	...
	case FU_DELL_KESTREL_EC_DEV_TYPE_PD:
		if (sub_type == FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI) {
			if (instance == FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP5)
				return "PD UP5";
			if (instance == FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP15)
				return "PD UP15";
			if (instance == FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP17)
				return "PD UP17";
		}
		return NULL;
	case FU_DELL_KESTREL_EC_DEV_TYPE_USBHUB:
		if (sub_type == FU_DELL_KESTREL_EC_DEV_USBHUB_SUBTYPE_RTS0)
			return "USB Hub RTS0";
		if (sub_type == FU_DELL_KESTREL_EC_DEV_USBHUB_SUBTYPE_RTS5)
			return "USB Hub RTS5";
		return NULL;
	case FU_DELL_KESTREL_EC_DEV_TYPE_MST:
		if (sub_type == FU_DELL_KESTREL_EC_DEV_MST_SUBTYPE_VMM8)
			return "MST VMM8";
		if (sub_type == FU_DELL_KESTREL_EC_DEV_MST_SUBTYPE_VMM9)
			return "MST VMM9";
		return NULL;
	case FU_DELL_KESTREL_EC_DEV_TYPE_TBT:
		if (sub_type == FU_DELL_KESTREL_EC_DEV_TBT_SUBTYPE_TR)
			return "TR";
		if (sub_type == FU_DELL_KESTREL_EC_DEV_TBT_SUBTYPE_GR)
			return "GR";
		if (sub_type == FU_DELL_KESTREL_EC_DEV_TBT_SUBTYPE_BR)
			return "BR";
		return NULL;
...
fu_dell_kestrel_ec_write(FuDevice *device, GByteArray *buf, GError **error)
{
	...
	if (!fu_dell_kestrel_ec_hid_i2c_write(device, buf->data, buf->len, error)) {
...
fu_dell_kestrel_ec_probe_pd(FuDevice *ec_dev,
			    FuDellKestrelEcDevType dev_type,
			    FuDellKestrelEcDevPdSubtype subtype,
			    guint8 instance,
			    GError **error)
{
...
fu_dell_kestrel_ec_probe_subcomponents(FuDevice *device, GError **error)
{
	...
	if (!fu_dell_kestrel_ec_probe_pd(device,
					 ...
					 FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI,
					 FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP5,
					 error))
		return FALSE;
	...
	if (!fu_dell_kestrel_ec_probe_pd(device,
					 FU_DELL_KESTREL_EC_DEV_TYPE_PD,
					 FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI,
					 FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP15,
					 error))
		return FALSE;
	...
	if (!fu_dell_kestrel_ec_probe_pd(device,
					 FU_DELL_KESTREL_EC_DEV_TYPE_PD,
					 FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI,
					 FU_DELL_KESTREL_EC_DEV_PD_SUBTYPE_TI_INSTANCE_UP17,
					 error))
		return FALSE;
...
fu_dell_kestrel_ec_set_dock_sku(FuDevice *device, GError **error)
{
	...
	case FU_DELL_DOCK_BASE_TYPE_KESTREL:
		...
		if (fu_dell_kestrel_ec_dev_entry(device,
						 ...
						 FU_DELL_KESTREL_EC_DEV_TBT_SUBTYPE_BR,
						 0) != NULL) {
		...
		if (fu_dell_kestrel_ec_dev_entry(device,
						 FU_DELL_KESTREL_EC_DEV_TYPE_TBT,
						 FU_DELL_KESTREL_EC_DEV_TBT_SUBTYPE_GR,
						 0) != NULL) {
...
guint32
fu_dell_kestrel_ec_get_pd_version(FuDevice *device, guint8 sub_type, guint8 instance)
{
	struct FuDellKestrelEcQueryEntry *dev_entry = NULL;
	FuDellKestrelEcDevType dev_type = FU_DELL_KESTREL_EC_DEV_TYPE_PD;

	dev_entry = fu_dell_kestrel_ec_dev_entry(device, dev_type, sub_type, instance);
...
fu_dell_kestrel_ec_get_chunk_size(FuDellKestrelEcDevType dev_type)
{
	...
	switch (dev_type) {
	case FU_DELL_KESTREL_EC_DEV_TYPE_MAIN_EC:
		return DELL_KESTREL_EC_DEV_EC_CHUNK_SZ;
	case FU_DELL_KESTREL_EC_DEV_TYPE_RMM:
		return DELL_KESTREL_EC_DEV_NO_CHUNK_SZ;
	default:
		return DELL_KESTREL_EC_DEV_ANY_CHUNK_SZ;
	}
}

static guint
fu_dell_kestrel_ec_get_first_page_delaytime(guint8 dev_type)
{
					...
					GError **error)
{
	...
	for (guint j = 0; j < fu_chunk_array_length(pages); j++) {
		guint8 page_aligned[DELL_KESTREL_EC_HID_DATA_PAGE_SZ] = {0xff};
		...
		/* send to ec */
		if (!fu_dell_kestrel_ec_hid_write(device, page_bytes, &error_local)) {
...
{
	guint8 res[DELL_KESTREL_EC_HID_DATA_PAGE_SZ] = {0xff};
					 ...
					 GError **error)
{
	...
	for (guint i = 0; i < fu_chunk_array_length(chunks); i++) {
		...
		pages = fu_chunk_array_new_from_bytes(buf,
						      FU_CHUNK_ADDR_OFFSET_NONE,
						      FU_CHUNK_PAGESZ_NONE,
						      DELL_KESTREL_EC_HID_DATA_PAGE_SZ);
...
fu_dell_kestrel_ec_init(FuDellKestrelEc *self)
{
	...
	fu_device_register_private_flag(FU_DEVICE(self),
					FWUPD_DELL_KESTREL_DEVICE_PRIVATE_FLAG_UOD_OFF);
